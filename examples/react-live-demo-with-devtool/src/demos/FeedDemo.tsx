 ...
 *
 * This copy of the demo also attaches `@scrollstackjs/devtools` to the engine —
 * the floating panel in the corner is reading this feed.
 */
export function FeedDemo(): React.ReactElement {
  const {
    ...
    // One line, and the panel follows this engine's whole life — it mounts after
    // React commits and unmounts when the adapter destroys the engine. In a real
    // app: `plugins: import.meta.env.DEV ? [devtoolsPlugin()] : []`.
    plugins: [devtoolsPlugin({ position: 'bottom-right', open: true })],
  })
